const docs = {
  ...
  mindmapVi: '../../../doc/chapter6/mind_map_chapter6_vi.md'
};
...
function renderMindmap(markdown) {
  ...
  const start = sourceLines.findIndex((line) => line.trim() === 'mindmap');
  const end = start >= 0 ? sourceLines.findIndex((line, index) => index > start && line.trim() === '```') : -1;
  const lines = (start >= 0 ? sourceLines.slice(start + 1, end >= 0 ? end : sourceLines.length) : sourceLines).filter((line) => line.trim());
  const root = { children: [] };
  const stack = [root];
  lines.forEach((line) => {
  ...
  walk(root.children[0] || { text: 'Mind Map', children: [] }, 0, 24);
  ...
  const nodes = positions.map((position) => {
    ...
    const connector = parent ? `<line x1="${parent.x + parent.width / 2}" y1="${parent.y + parent.height}" x2="${position.x + position.width / 2}" y2="${position.y}" stroke="#38bdf8" stroke-width="2" stroke-dasharray="4 4" />` : '';
    return `${connector}<rect x="${position.x}" y="${position.y}" width="${position.width}" height="${position.height}" rx="12" fill="#111827" stroke="#38bdf8" stroke-width="1.5" /><text x="${position.x + position.width / 2}" y="${position.y + 34}" fill="#f8fafc" font-size="12" font-weight="600" text-anchor="middle">${escapeHtml(position.text)}</text>`;
  }).join('');
  return `<div class="mindmap"><svg viewBox="0 0 ${maxX + 40} ${maxY + 40}" xmlns="http://www.w3.org/2000/svg">${nodes}</svg></div>`;
}
...
function parseQuiz(markdown) {
  ...
  markdown.split(/\r?\n/).forEach((rawLine) => {
    const line = rawLine.trim();
    if (!line) return;
    if (/^###\s+Question\s+\d+/.test(line)) { push(); current = { prompt: line.replace(/^###\s+Question\s+\d+/, '').trim(), choices: [], answer: '', explanation: '' }; mode = 'prompt'; }
    else if (current && /^([A-D])\.\s+/.test(line)) { const match = line.match(/^([A-D])\.\s+(.*)$/); current.choices.push({ label: match[1], text: match[2] }); mode = 'choices'; }
