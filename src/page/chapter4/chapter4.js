const docs = {
  ...
  quiz1: '../../../doc/chapter4/Chapter4_Quiz.md',
  quiz2: '../../../doc/chapter4/Chapter4_Quiz_2.md',
  quiz3: '../../../doc/chapter4/Chapter4_Quiz_3.md',
...
function renderGlossary(markdown) {
  ...
  if (tableLines.length < 2) return '<p>No glossary entries were found.</p>';
  const rows = tableLines.slice(2).map((line) => line.split('|').slice(1, -1).map((cell) => cell.trim()));
  return `<div class="glossary-grid">${rows.filter((row) => row.length >= 2).map(([term, meaning]) => `<article class="glossary-card"><div class="glossary-term">${escapeHtml(term)}</div><div class="glossary-meaning">${escapeHtml(meaning)}</div></article>`).join('')}</div>`;
}
...
async function loadDoc(view) {
  ...
  try {
    ...
    if (view.startsWith('quiz')) {
      ...
      contentArea.innerHTML = renderQuiz(questions);
      bindQuiz(questions);
    } else if (view === 'glossary') {
      contentArea.innerHTML = renderGlossary(markdown);
    } else if (view === 'mindmap') {
      contentArea.innerHTML = renderMindmap(markdown);
    } else {
      contentArea.innerHTML = renderMarkdown(markdown);
    }
  } catch (error) {
    contentArea.innerHTML = `<div class="notice">Could not load this chapter view. ${escapeHtml(error.message)}</div>`;
  }
}
...
buttons.forEach((button) => button.addEventListener('click', () => {
  ...
  const view = button.dataset.view === 'en' || button.dataset.view === 'vi' ? currentLanguage : button.dataset.view;
  loadDoc(view);
}));
